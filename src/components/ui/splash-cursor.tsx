...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvasElement: HTMLCanvasElement) {
      ...
      if (!gl) {
        gl = (canvasElement.getContext('webgl', params) ||
    ...
    function updateColors(dt: number, colorUpdateTimer: number) {
      let timer = colorUpdateTimer + dt * config.COLOR_UPDATE_SPEED;
      if (timer >= 1) {
        timer = wrap(timer, 0, 1);
        pointers.forEach((p) => {
    ...
    function updateFrame() {
      ...
      colorUpdateTimer = updateColors(dt, colorUpdateTimer);
      applyInputs();
      step(dt);
      render(null);
